...
const projects = [
  ...
  },
  {
    ...
    desc: "Complete e-commerce storefront + admin panel. Full-stack with Supabase backend, auth, and PostgreSQL queries.",
...
function ProjectCard({ project, index }: { project: typeof projects[0]; index: number }) {
  ...
  return (
    <article
      className={`project-card group relative rounded-2xl glass border transition-all duration-400 overflow-hidden flex flex-col
    ...
    >
      ...
      <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none
        ${isPink ? "bg-gradient-to-br from-[rgba(247,37,133,0.06)] via-transparent to-transparent" : "bg-gradient-to-br from-[rgba(76,201,240,0.06)] via-transparent to-transparent"}`}
        aria-hidden="true" />

      {/* Number badge */}
      <div className={`absolute top-[var(--space-5)] right-[var(--space-5)] font-mono text-xs font-bold opacity-20 ${isPink ? "text-[var(--color-pink)]" : "text-[var(--color-blue)]"}`} aria-hidden="true">
        {String(index + 1).padStart(2, "0")}
      </div>

      <div className={`p-[var(--space-6)] ${isFeatured ? "md:p-[var(--space-10)]" : ""} flex flex-col gap-[var(--space-4)] flex-1`}>
        <h3 className={`font-display font-bold leading-tight ${isFeatured ? "text-2xl md:text-3xl" : "text-xl"}`}
          style={{ color: isPink ? "var(--color-pink)" : "var(--color-blue)" }}>
          {project.title}
        </h3>
        {project.highlight && (
          <p className="font-mono text-xs text-[var(--color-muted)] italic border-l-2 border-[var(--color-pink)] pl-[var(--space-3)]">
            {project.highlight}
          </p>
        )}
        <p className="font-body text-[var(--color-muted)] text-sm leading-relaxed flex-1">{project.desc}</p>
        <div className="flex flex-wrap gap-[var(--space-2)]">
          {project.tags.map((t) => (
            <span key={t} className={`px-2.5 py-1 rounded-full text-xs font-mono ${isPink
        ...
        <div className="flex items-center gap-[var(--space-4)] pt-[var(--space-2)]">
          {project.demo && (
            <a href={project.demo} target="_blank" rel="noopener noreferrer" data-cursor="Open"
              className={`flex items-center gap-1.5 text-sm font-body font-medium transition-colors ${isPink ? "text-[var(--color-pink)] hover:opacity-70" : "text-[var(--color-blue)] hover:opacity-70"}`}
              aria-label={`Live demo of ${project.title}`}>
              <ExternalLink size={14} aria-hidden="true" /> Live Demo ↗
            </a>
          )}
          {project.github && (
            <a href={project.github} target="_blank" rel="noopener noreferrer" data-cursor="Code"
...
export default function Projects() {
  ...
  const bgGlow = (
    <div style={{ background: "radial-gradient(ellipse 60% 50% at 20% 50%, rgba(247,37,133,0.04) 0%, transparent 70%), radial-gradient(ellipse 50% 40% at 80% 70%, rgba(76,201,240,0.04) 0%, transparent 70%)", inset: 0, position: "absolute" }} />
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-[var(--space-4)]">
        <div>
          <p className="font-mono text-sm text-[var(--color-pink)] mb-[var(--space-3)] tracking-widest uppercase">Portfolio</p>
