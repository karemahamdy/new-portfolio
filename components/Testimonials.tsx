...
function TestimonialCard({ t }: { t: typeof testimonials[0] }) {
  return (
    <div className="flex-shrink-0 w-80 glass rounded-2xl border border-[var(--color-border)] p-6 mx-4">
      <Quote size={22} className="text-[var(--color-pink)] mb-3 opacity-70" aria-hidden="true" />
      <p className="font-body text-sm text-[var(--color-muted)] leading-relaxed mb-5 italic">&ldquo;{t.text}&rdquo;</p>
      <div className="flex items-center gap-1 mb-3" aria-label={`${t.stars} stars`}>
        {Array.from({ length: t.stars }).map((_, i) => (
          <Star key={i} size={12} className="text-yellow-400 fill-yellow-400" aria-hidden="true" />
        ))}
      </div>
      <div className="h-px bg-gradient-to-r from-[var(--color-pink)] to-[var(--color-blue)] mb-4 opacity-30" aria-hidden="true" />
      <div>
...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="relative py-32 overflow-hidden" aria-label="Testimonials">
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true"
        style={{ background: "radial-gradient(ellipse 50% 40% at 50% 50%, rgba(247,37,133,0.04) 0%, transparent 70%)" }} />

      <div className="max-w-7xl mx-auto px-6 mb-14">
        <div className="text-center">
          <p className="font-mono text-sm text-[var(--color-blue)] mb-3 tracking-widest uppercase">Testimonials</p>
